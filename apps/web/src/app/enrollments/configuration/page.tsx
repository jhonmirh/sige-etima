...
export default function EnrollmentConfiguration() {
  ...
  return <Shell title="Configuración de matrícula">
    ...
    {me?.role === 'ADMIN' && <section className="card form-section" style={{marginTop:16}}>
      <div className="section-head"><div><span className="eyebrow">Solo cuando el plan lo requiera</span><h2>Catálogo de menciones</h2><p>Las menciones solo pueden agregarse a planes configurados con “Tiene mención”. La modalidad y el código siempre provienen del plan asociado.</p></div><School size={22}/></div>
      <form onSubmit={createMention} className="form-grid cols-3" style={{alignItems:'end',marginBottom:16}}>
        <div><label>Plan de estudio *</label><select className="input" name="studyPlanId" required><option value="">Seleccione</option>{catalogPlans.filter((p:any)=>p.hasMention).map((p:any)=><option key={p.id} value={p.id}>{p.code} · {p.modality==='MEDIA_TECNICA'?'MEDIA TÉCNICA':'MEDIA GENERAL'} · {p.optionName || p.name}</option>)}</select></div>
        ...
        <button className="btn" type="submit" disabled={catalogPlans.filter((p:any)=>p.hasMention).length===0}><Plus size={16}/> Agregar mención</button>
      </form>
      <div className="table-wrap"><table><thead><tr><th>Modalidad</th><th>Plan</th><th>Mención</th><th>Estado</th><th>Secciones</th><th>Acciones</th></tr></thead><tbody>{mentions.filter((m:any)=>m.studyPlan?.hasMention).length===0?<tr><td colSpan={6}>Todavía no hay menciones registradas para planes que las requieran.</td></tr>:mentions.filter((m:any)=>m.studyPlan?.hasMention).map((m:any)=><tr key={m.id}><td><strong>{m.studyPlan?.modality==='MEDIA_TECNICA'?'MEDIA TÉCNICA':'MEDIA GENERAL'}</strong></td><td>{m.studyPlan?.code}</td><td><strong>{m.name}</strong></td><td><span className={`status ${m.active?'ok':'neutral'}`}>{m.active?'ACTIVA':'INACTIVA'}</span></td><td>{m._count?.sections||0}</td><td><div className="row-actions"><button className="btn secondary mini-btn" type="button" onClick={()=>renameMention(m)}>Renombrar</button><button className="btn secondary mini-btn" type="button" onClick={()=>toggleMention(m.id,!m.active)}>{m.active?'Inactivar':'Activar'}</button></div></td></tr>)}</tbody></table></div>
