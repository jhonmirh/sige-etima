...
import {dateLabel,schoolDateKey,rosterLockDateKeyFromClose,rosterLockLabelFromClose} from '@/lib/schoolCalendar';
...
export default function Enrollments(){
  ...
  const closeReached=!!year?.enrollmentCloseDate && schoolDateKey()>=rosterLockDateKeyFromClose(year.enrollmentCloseDate);
  const rosterLockLabel=year?.enrollmentCloseDate?rosterLockLabelFromClose(year.enrollmentCloseDate):'01/11/AAAA';
  ...
  return <Shell title="Matrícula / Reinscripción">
    ...
    {year&&<>
      ...
      {year.enrollmentCloseDate&&<div className={closeReached?'success-banner':'warning-banner'} style={{marginTop:12}}><div><strong>{closeReached?'NÓMINA FIJA AUTOMÁTICAMENTE':'NÓMINA PROVISIONAL HASTA EL 31 DE OCTUBRE'}</strong><span>{closeReached?`Desde el ${rosterLockLabel} los números ocupados no cambian. Los retiros posteriores conservan su posición y toda inscripción o reinscripción nueva se agrega al final por fecha/hora de registro.`:`Hasta el ${dateLabel(year.enrollmentCloseDate)} inclusive la nómina se ordena por número de cédula dentro de cada año/grado, plan o mención y sección. Un retiro en este período sale de la nómina y ésta se reordena por cédula. La numeración queda fija desde el ${rosterLockLabel}.`}{fixedCount>0?` · ${fixedCount} registro(s) visibles con numeración fija.`:''}</span></div></div>}
    </>}
    <div className="table-wrap"><table><thead><tr><th>N°</th><th>Identificación</th><th>Estudiante</th><th>Plan</th><th>Mención</th><th>Grado / Sección</th><th>Condición</th><th>Materias</th><th>Inscripción</th><th>Acción</th></tr></thead><tbody>{loading?<tr><td colSpan={10}>Cargando…</td></tr>:rows.length===0?<tr><td colSpan={10}>No hay matrículas con los filtros seleccionados.</td></tr>:rows.map(e=><tr key={e.id}><td><strong>{e.displayListNumber??e.listNumber??'—'}</strong>{e.rosterStatus==='PROVISIONAL'&&<small className="muted" style={{display:'block'}}>PROV.</small>}{e.rosterStatus==='FUERA_DE_NOMINA'&&<small className="warning-text" style={{display:'block'}}>FUERA DE NÓMINA</small>}</td><td>{idLabel(e.student)}</td><td><Link className="table-link" href={`/enrollments/${e.id}`}>{e.student.lastName} {e.student.secondLastName||''}, {e.student.firstName} {e.student.middleName||''}</Link></td><td>{e.studyPlan.code}</td><td>{e.section.mentionName||'SIN DEFINIR'}</td><td>{e.gradeLevel}° · {e.section.name}</td><td><span className={`status ${e.condition==='REGULAR'?'ok':e.condition==='MATERIA_PENDIENTE'?'warn':'neutral'}`}>{pretty(e.condition)}</span></td><td>{e.curriculumSubjects?.length||0}</td><td>{new Date(e.registrationDate).toLocaleDateString('es-VE')}{e.isLateEnrollment?' · POST-CIERRE':''}</td><td><Link className="btn secondary mini-btn" href={e.withdrawal?`/enrollments/${e.id}`:`/enrollments/${e.id}#retiro`}>{e.withdrawal?'Ver retiro':'Gestionar / Retiro'}</Link></td></tr>)}</tbody></table></div>
  </Shell>
}
