...
export default function LinkRepresentativeToStudent(){
  ...
  async function loadData(){
    try{
      const [s,reps]:any[]=await Promise.all([api(`/students/${id}`),api('/representatives')]);
  ...
  const linkedIds=useMemo(()=>new Set((student?.representatives||[]).map((x:any)=>x.representativeId)),[student]);
  const availableCount=visibleRepresentatives.filter((r:any)=>r.active!==false&&!linkedIds.has(r.id)).length;
  ...
  return <Shell title="Vincular representante">
    ...
    <div className="card" style={{marginBottom:16}}>
      ...
      <p className="muted">Registrados: {representatives.length} · Ya vinculados: {student.representatives?.length||0} · Disponibles para vincular: {availableCount}</p>
    </div>
    ...
    <form onSubmit={submit} className="stack">
      <section className="card form-section">
        ...
        {visibleRepresentatives.length===0?<div className="empty-state"><UsersRound size={28}/><strong>No se encontraron representantes</strong><span>No hay coincidencias con la búsqueda actual.</span><Link className="btn" href={`/representatives/new?studentId=${id}`}><UserRoundPlus size={16}/> Crear representante</Link></div>:
        <div className="table-wrap"><table><thead><tr><th></th><th>Cédula</th><th>Representante</th><th>Teléfono</th><th>Estado del vínculo</th><th>Estudiantes asociados</th></tr></thead><tbody>{visibleRepresentatives.map((r:any)=>{const alreadyLinked=linkedIds.has(r.id);const inactive=r.active===false;const selectable=!alreadyLinked&&!inactive;return <tr key={r.id} onClick={()=>selectable&&setSelected(r.id)} style={{cursor:selectable?'pointer':'default',opacity:inactive?0.65:1}}><td><input type="radio" name="representativeChoice" checked={selected===r.id} onChange={()=>selectable&&setSelected(r.id)} disabled={!selectable}/></td><td><strong>{identityLabel(r)}</strong></td><td>{[r.firstName,r.middleName,r.lastName,r.secondLastName].filter(Boolean).join(' ')}</td><td>{r.phone1}</td><td>{alreadyLinked?<span className="status ok">YA VINCULADO</span>:inactive?<span className="status warn">INACTIVO</span>:<span className="status neutral">DISPONIBLE</span>}</td><td>{r.students?.length||0}</td></tr>})}</tbody></table></div>}
      </section>
